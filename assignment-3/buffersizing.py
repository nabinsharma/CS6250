...
class StarTopo(Topo):
    ...
    def create_topology(self):
        host1 = self.addHost("host1", cpu=self.cpu)
        host2 = self.addHost("host2", cpu=self.cpu)
        host3 = self.addHost("host3", cpu=self.cpu)
        s1 = self.addSwitch("s1")
        self.addLink(host1, s1, bw=self.bw_host, delay=self.delay,
                     loss=0, max_queue_size=self.maxq)
        self.addLink(host2, s1, bw=self.bw_host, delay=self.delay,
                     loss=0, max_queue_size=self.maxq)
        self.addLink(s1, host3, bw=self.bw_net, delay=self.delay,
                     loss=0, max_queue_size=self.maxq)
...
def do_sweep(iface):
    ...
    while abs(min_q - max_q) >= 2:
        ...
        if ok(median(rates) / reference_rate):
            max_q = mid - 1
        else:
            min_q = mid + 1
...
# Notice: I just printed the commands so that I can see if the
# latency and bandwidth are in the expected range. This is just
# a visual check. No logging.

# TODO: Fill in the following function to verify the latency
# settings of your topology.

def verify_latency(net):
    print 'Verifying (in fact, just printing) latency ...'
    h1 = net.getNodeByName("host1")
    h2 = net.getNodeByName("host2")
    h3 = net.getNodeByName("host3")
    print h1.cmd('ping -c 2 %s' % h3.IP())
    print h2.cmd('ping -c 2 %s' % h3.IP())
...
def verify_bandwidth(net):
    print 'Verifying (in fact, just printing) bandwidth ...'
    server = net.getNodeByName("host3")
    client = net.getNodeByName("host1")
    server.cmd(CUSTOM_IPERF_PATH, '-s &')
    client.sendCmd(CUSTOM_IPERF_PATH, '-c', server.IP())
    print client.waitOutput().strip()
    server.cmd('kill $!')
...
def start_senders(net):
    ...
    h2 = net.getNodeByName("host2")
    h3 = net.getNodeByName("host3")
    cmd1 = '%s -c %s -p %s -t %d -i 1 -yc -Z%s > %s/%s &' % (
        CUSTOM_IPERF_PATH, h3.IP(), 5001, seconds, args.cong, args.dir,
        'iperf_client_1.txt')
    cmd2 = '%s -c %s -p %s -t %d -i 1 -yc -Z%s > %s/%s &' % (
        CUSTOM_IPERF_PATH, h3.IP(), 5001, seconds, args.cong, args.dir,
        'iperf_client_2.txt')
...
def main():
    ...
    topo = StarTopo(n=NUM_HOSTS, bw_host=args.bw_host,
                    delay='%sms' % (args.delay/2.0),
                    bw_net=args.bw_net, maxq=args.maxq)
    net = Mininet(topo=topo, host=CPULimitedHost, link=TCLink,
                  autoPinCpus=True)
    
    s1 = net.getNodeByName("s1")
    s1.sendCmd('tc qdisc; tc class show dev s1-eth3')
    result = s1.waitOutput()
    print(result)
