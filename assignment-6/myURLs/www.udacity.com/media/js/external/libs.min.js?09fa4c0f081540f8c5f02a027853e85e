...
b=0;return a=a.replace(d,function(a){a=j[b]?"":a;b++;return a})}var d,e;"object"===typeof exports&&"function"===typeof require?(d=exports,e=require("./Markdown.Converter").Converter):(d=window.Markdown,e=d.Converter);d.getSanitizingConverter=function(){var a=new e;a.hooks.chain("postConversion",l);a.hooks.chain("postConversion",s);return a};var n=/^(<\/?(b|blockquote|code|del|dd|dl|dt|em|h1|h2|h3|i|kbd|li|ol|p|pre|s|sup|sub|strong|strike|ul)>|<(br|hr)\s?\/?>)$/i,q=/^(<a\s(\s?target\s?=\s?"_blank")?\s?href="((https?|ftp):\/\/|\/)[-A-Za-z0-9+&@#\/%?=~_|!:,.;\(\)]+"(\starget\s?=\s?"_blank")?\s?(\stitle="[^"<>]+")?\s?>|<\/a>)$/i,
