...
class Topo1(Topo):
    ...
    def __init__(self, n=1, cpu=.1, bw=100, delay=None,
        ...
        # TODO
        host_config = {'cpu': cpu}
        link_config_eth = {'bw': 10, 'delay': '1ms', 'loss': 0,
                           'max_queue_size': max_queue_size }
        link_config_wifi = {'bw': 2, 'delay': '5ms', 'loss': 3,
                            'max_queue_size': max_queue_size }
        link_config_3g = {'bw': 2, 'delay': '75ms', 'loss': 2,
                          'max_queue_size': max_queue_size }
	...
	sender = self.addHost('sender', **host_config)
	receiver = self.addHost('receiver', **host_config)

        # Wire receiver
        # TODO
        self.addLink(receiver, s1,
                     port1=0, port2=downlink, **link_config_eth)
        self.addLink(receiver, s2,
                     port1=1, port2=downlink, **link_config_wifi)
        self.addLink(receiver, s3,
                     port1=2, port2=downlink, **link_config_3g)

        # Wire sender
        # TODO
        self.addLink(sender, s1,
                     port1=0, port2=uplink, **link_config_eth)
	self.addLink(sender, s2,
                     port1=1, port2=uplink, **link_config_eth)
	self.addLink(sender, s3,
                     port1=2, port2=uplink, **link_config_eth)
